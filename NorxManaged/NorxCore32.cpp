...
{                                                   \
	...
	//These BURN methods should survive compiler optimization, they are only used at the end of a processes
		...
		static void _encrypt_p1(array<UInt32>^ state, array<const Byte>^ in, const _domain_separator tag, const Byte rounds, array<Byte>^ out)
		{
			...
			for (Int64 i = 0; i < in->LongLength; i += NORX32_RATEBYTES)
			{
				...
				if (last)
					Buffer::BlockCopy(state, 0, out, outptr, in->LongLength % NORX32_RATEBYTES);
				else
		...
		static void _decrypt_p1(array<UInt32>^ state, array<const Byte>^ in, const _domain_separator tag, const Byte rounds, array<Byte>^ out)
		{
			// Used only for Payload of Parallelism > 1 (not P=0)
			if (in->LongLength == 0) return;
			Int64 outptr = 0;
			...
			for (Int64 i = 0; i < in->LongLength; i += NORX32_RATEBYTES)
			{
				...
				if (last)
				{
					...
					if (i < in->LongLength)
						Buffer::BlockCopy(in, i, LastBlock, 0, in->LongLength % NORX32_RATEBYTES);
					LastBlock[in->LongLength % NORX32_RATEBYTES] ^= 0x01; // remove the padding
					LastBlock[LastBlock->Length - 1] ^= 0x80;
					Buffer::BlockCopy(LastBlock, 0, state_buffer, 0, NORX32_RATEBYTES);
				}
				else
				...
				if (last)
					Buffer::BlockCopy(state_buffer, 0, out, outptr, in->LongLength % NORX32_RATEBYTES);
				else
		...
		static void _encrypt_p2(array<array<UInt32>^>^ states, array<const Byte>^ in, const _domain_separator tag, const Byte rounds, const Byte lanes, array<Byte>^ out)
		{
			// Used only for Payload of Parallelism > 1 (not P=0)
			if (in->LongLength == 0) return;
			Int64 outptr = 0;
			...
			for (Int64 i = 0; i < in->LongLength; i += NORX32_RATEBYTES)
			{
				...
				{
					if (last)
						Buffer::BlockCopy(states[laneptr], 0, out, outptr, in->LongLength % NORX32_RATEBYTES);
					else
		...
		static void _decrypt_p2(array<array<UInt32>^>^ states, array<const Byte>^ in, const _domain_separator tag, const Byte rounds, const Byte lanes, array<Byte>^ out)
		{
			// Used only for Payload of Parallelism > 1 (not P=0)
			if (in->LongLength == 0) return;
			Int64 outptr = 0;
		...
		static __inline void _finalize(array<UInt32>^ state, array<const UInt32>^ k, const Byte rounds, const short tagsizebits, array<Byte>^ out)
		{
			state[15] ^= FINAL_TAG;
			...
			Buffer::BlockCopy(state, NORX32_RATEWORDS * NORX32_WORDBYTES, out, 0, tagsizebits / 8);
