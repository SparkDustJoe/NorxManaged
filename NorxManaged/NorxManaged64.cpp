...
namespace NorxManaged
{
		...
		[Out] array<Byte>^% Output)
	{
		...
		else if (Parallelism > 1)
		{
			...
			for (Byte i = 0; i < Parallelism; i++)
			{
				...
				NorxCore64::_branch(lanes[i], i, Rounds);
			}
		...
		[Out] array<Byte>^% Output)
	{
		...
		if (NorxCore64::norx_verify_tag((array<const Byte>^)inputTag, (array<const Byte>^)tempTag) == 0)
			returnValue = 0; // OK
		else
		{
		...
		[Out] array<Byte>^% Output)
	{
		...
		kt = nullptr;
		int returnValue = -1;
		if (NorxCore64::norx_verify_tag(Tag, (array<const Byte>^)tempTag) == 0)
			returnValue = 0; // ok
		else
		{
			NorxCore64::_burn(Output);
